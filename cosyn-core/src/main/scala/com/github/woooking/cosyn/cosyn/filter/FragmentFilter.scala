...
case class FragmentFilter(dfgNode: DFGNode) extends SeqFilter[Fragment[DFGNode, DFGEdge]] {
    ...
    override def filter(data: Seq[Fragment[DFGNode, DFGEdge]]): Seq[Fragment[DFGNode, DFGEdge]] = data.filter(valid)
}
