...
object FillHoleVisitor {
    ...
    def instance[A](implicit enc: FV[A]): FV[A] = enc

    def create[A](func: (A, HoleExpr, Expression) => Option[A]): FV[A] = (node: A, hole: HoleExpr, expr: Expression) => func(node, hole, expr)

    def nil[A]: FV[A] = create((_, _, _) => None)

    implicit def valInstance[V <: AnyVal]: FV[V] = nil

    implicit def stringInstance: FV[String] = nil

    implicit def exprInstance[R](implicit generic: Generic.Aux[Expression, R], lazyInstance: Lazy[FV[R]]): FV[Expression] = create { (value, hole, expr) =>
        value match {
    ...
    implicit def seqInstance[V](implicit vInstance: Lazy[FV[V]]): FV[Seq[V]] = create { (seq, hole, expr) =>
    ...
    implicit def optionInstance[V](implicit vInstance: Lazy[FV[V]]): FV[Option[V]] = create {
        case (None, _, _) => None
    ...
    implicit def hListInstance[H, T <: HList](implicit hInstance: Lazy[FV[H]], tInstance: FV[T]): FV[H :: T] = create {
        case (h :: t, hole, expr) =>
    ...
    implicit def coproductInstance[H, T <: Coproduct](implicit hInstance: Lazy[FV[H]], tInstance: FV[T]): FV[H :+: T] = create {
        case (Inl(h), hole, expr) => hInstance.value.fill(h, hole, expr).map(Inl(_))
        case (Inr(t), hole, expr) => tInstance.fill(t, hole, expr).map(Inr(_))
    }

    implicit def genericInstance[A, R](implicit generic: Generic.Aux[A, R], rInstance: Lazy[FV[R]]): FV[A] = create { (value, hole, expr) =>
