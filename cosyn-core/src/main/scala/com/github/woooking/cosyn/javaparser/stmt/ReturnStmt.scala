...
class ReturnStmt(override val delegate: JPReturnStmt) extends Statement {
    val expression: Option[Expression[_ <: Node]] = delegate.getExpression.asScala.map(e => Expression(e))
}

object ReturnStmt {
    def apply(delegate: JPReturnStmt): ReturnStmt = new ReturnStmt(delegate)

    def unapply(arg: ReturnStmt): Option[Option[Expression[_ <: Node]]] = Some(arg.expression)
}