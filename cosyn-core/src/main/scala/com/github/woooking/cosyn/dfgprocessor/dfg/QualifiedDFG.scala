...
import com.github.woooking.cosyn.dfgprocessor.ir.statements.{IRMethodInvocation, IRStatement}
...
class QualifiedDFG(val cfg: CFGImpl) extends ListGraph[DFGNode, DFGEdge] {
...
object QualifiedDFG {
    implicit val dfgPrintable = new Printable[QualifiedDFG] {
        override def print(obj: QualifiedDFG, ps: PrintStream): Unit = {
    ...
    def apply(cfg: CFGImpl): QualifiedDFG = {
        val dfg = new QualifiedDFG(cfg)
