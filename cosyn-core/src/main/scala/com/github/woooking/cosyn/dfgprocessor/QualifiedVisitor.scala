...
class QualifiedVisitor(javaParserFacade: JavaParserFacade = null) {
    ...
    def visitExpression(cfg: CFGImpl)(block: CFGStatements, node: Expression[_ <: Node]): IRExpression = node match {
        ...
        case n@MethodCallExpr(_, scope, _, arguments) =>
            val (name, qualified) = qualifiedMethodCall(n)
    ...
    private def qualifiedMethodCall(methodCall: MethodCallExpr): (String, Boolean) = {
        try {
