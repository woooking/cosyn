...
class ReturnStmt(override val delegate: JPReturnStmt) extends Statement {
    val expression: Option[Expression] = delegate.getExpression.asScala.map(e => Expression(e))
}

object ReturnStmt {
    def apply(delegate: JPReturnStmt): ReturnStmt = new ReturnStmt(delegate)

    def unapply(arg: ReturnStmt): Option[Option[Expression]] = Some(arg.expression)
}