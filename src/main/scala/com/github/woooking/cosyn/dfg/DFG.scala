...
class DFG(val cfg: CFG) extends ListGraph[DFGNode, DFGEdge] with Printable {
    ...
    def isSuperGraph(graph: Graph[DFGNode, DFGEdge]): Boolean = {
    ...
    def isSuperGraph(nodeMap: BiMap[DNode, DNode], current: DNode, remain: List[DNode]): Boolean = {
        var result = false
        ...
        while (ite.hasNext && !result) {
            ...
            if (!inDiff && !outDiff) {
                ...
                remain match {
                    ...
                    case n :: ns =>
                        if (isSuperGraph(nodeMap, n, ns)) result = true
                        nodeMap.remove(node)
                }
            }
        }
