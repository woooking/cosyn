...
class IRMethodInvocation(cfg: CFGImpl, val name: String, val receiver: Option[IRExpression], val args: Seq[IRExpression], fromNode: Set[Node]) extends IRDefStatement(cfg, fromNode) {
...
object IRMethodInvocation {
    def apply(cfg: CFGImpl, name: String, receiver: Option[IRExpression], args: Seq[IRExpression], fromNode: Set[Node]): IRMethodInvocation =
